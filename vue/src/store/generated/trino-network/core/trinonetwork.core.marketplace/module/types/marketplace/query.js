...
const baseQueryGetNodeRequest = { id: 0 };
...
export class QueryClientImpl {
    ...
    Params(request) {
        ...
        const promise = this.rpc.request("trinonetwork.core.marketplace.Query", "Params", data);
        return promise.then((data) => QueryParamsResponse.decode(new Reader(data)));
    }
    Node(request) {
        const data = QueryGetNodeRequest.encode(request).finish();
        const promise = this.rpc.request("trinonetwork.core.marketplace.Query", "Node", data);
        return promise.then((data) => QueryGetNodeResponse.decode(new Reader(data)));
    }
    NodeAll(request) {
        const data = QueryAllNodeRequest.encode(request).finish();
        const promise = this.rpc.request("trinonetwork.core.marketplace.Query", "NodeAll", data);
